...
class DispatchServiceML {
  constructor() {
    this.maxDistanceKm = process.env.DISPATCH_MAX_DISTANCE_METERS
      ? parseInt(process.env.DISPATCH_MAX_DISTANCE_METERS, 10) / 1000
      : 10;
  }
  ...
  buildRecommendation(match) {
    ...
    if (onTimeRate >= 95) {
      reason.push('Outstanding on-time performance');
    } else if (onTimeRate >= 90) {
      reason.push('Strong on-time performance');
    }
    ...
    try {
      ...
      const queueWithScores = await Promise.all(
        unassignedOrders.map(async (order) => {
          ...
          let priorityScore = waitMinutes * 2;
  ...
  async initialize() {
    ...
    try {
      ...
      deliveryPredictionML.schedulePeriodicRetraining(1000 * 60 * 60);
